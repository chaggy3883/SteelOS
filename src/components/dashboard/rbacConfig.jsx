import { ListChecks, Calculator, TrendingUp, History, Plus, FolderKanban, FileEdit, Factory, Truck, DollarSign } from 'lucide-react';
...
export const BUILTIN_ROLES = [
  ...
  { name: 'estimator', label: 'Estimator', is_system: true, description: 'Estimating, Takeoff Engine, CRM', allowed_modules: ['/', '/estimating', '/projects', '/crm', '/intelligence', '/documents', '/rfis'], allowed_widgets: ['bid_list', 'active_bids_count', 'bid_win_rate', 'bid_history', 'quick_add_bid'] },
  { name: 'project_manager', label: 'Project Manager', is_system: true, description: 'Job Tracking, Change Orders, Logistics, Billing', allowed_modules: ['/', '/projects', '/production', '/shipping', '/accounting', '/documents', '/rfis', '/reports'], allowed_widgets: ['active_projects', 'change_orders', 'fab_progress', 'shipments_calendar', 'invoiced_vs_remaining'] },
  { name: 'purchasing_agent', label: 'Purchasing Agent', is_system: true, description: 'Purchasing & vendor management', allowed_modules: ['/', '/purchasing', '/crm', '/inventory'], allowed_widgets: [] },
...
export async function getUserPermissions(roleName) {
  const builtin = BUILTIN_ROLES.find(r => r.name === roleName);
  if (builtin) return { modules: builtin.allowed_modules, widgets: builtin.allowed_widgets };
  try {
    const custom = await base44.entities.CustomRole.filter({ role_name: roleName }, '-created_date', 1);
    if (custom.length > 0) {
